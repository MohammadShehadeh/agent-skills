...
At each level ask "does the bug still reproduce below this point?" and descend. Flat trees (the `agent-skills-conventions` skill's `components` rule) keep this walk short — if localizing takes more than a few jumps, that is itself a finding.
...
- **Inbound — who references it**: grep the file's import path. Direct imports with no barrels (`agent-skills-conventions`: `structure` rule) mean every consumer is one grep away: `rg "components/date-range-picker"` → the complete consumer list.
...
- The greppable log prefix (`'Error in <fn>::'`, `agent-skills-conventions`: `errors` rule) tells you which layer produced the bad value.
...
- Add the regression test where the decision lives (pure logic, `agent-skills-conventions`: `testing` rule), not where the symptom rendered.
